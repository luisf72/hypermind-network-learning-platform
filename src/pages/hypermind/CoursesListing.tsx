...
const courses: Course[] = [
  ...
  },
  {
    ...
    duration: '5h 40m',
    students: '9k',
    description: 'Composition, light, and seeing the world like a photographer — phone or camera.',
    price: '$15',
    verified: true,
  },
  {
...
const levels: { name: Course['level']; count: number }[] = [
  { name: 'Beginner', count: 5_412 },
  { name: 'Intermediate', count: 4_318 },
  { name: 'Advanced', count: 2_750 },
]
const durations = [
  { label: 'Under 3 hours', count: 1_240 },
  { label: '3 – 10 hours', count: 6_812 },
  { label: '10 – 25 hours', count: 3_510 },
...
export default function CoursesListing() {
  ...
  const [selectedLevels, setSelectedLevels] = useState<Set<string>>(new Set(['Beginner']))
  const [selectedDurations, setSelectedDurations] = useState<Set<string>>(new Set(['3 – 10 hours']))
  const [activeRating, setActiveRating] = useState<number>(4.5)
  ...
  return (
    <div className={`hm-root ${isLight ? 'hm-light' : ''}`}>
      <Header active="courses" theme={theme} onThemeToggle={toggleTheme} />
      ...
      <main className="mx-auto w-full max-w-[1280px] px-6 pt-14 pb-20">
        ...
        <section className="relative pb-10">
          ...
          <div className="relative text-center max-w-3xl mx-auto">
            ...
            <p
              className="text-[15.5px] max-w-xl mx-auto mb-8"
              style={{ color: 'var(--hm-text-muted)', lineHeight: 1.55 }}
            >
              Twelve disciplines, hand-picked instructors, and a verifiable certificate every time
              you finish.
            </p>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          {/* Sidebar filters */}
          <aside className="lg:col-span-3 flex flex-col gap-7">
            ...
            <FilterGroup label="DURATION">
              <div className="flex flex-col gap-2">
                {durations.map((d) => (
                  <CheckboxRow
                    key={d.label}
                    checked={selectedDurations.has(d.label)}
                    onChange={() => toggleSet(selectedDurations, d.label, setSelectedDurations)}
                    label={d.label}
                    count={d.count}
                  />
                ))}
              ...
              <div className="grid grid-cols-3 gap-1.5">
                {(['all', 'free', 'paid'] as const).map((p) => (
                  <button
                    ...
                    type="button"
                    onClick={() => setPrice(p)}
                    className="h-8 text-[12px] font-medium rounded-md capitalize transition-colors"
                    style={{
                  ...
                  return (
                    ...
                    >
                      <span className="flex items-center gap-1">
                        ...
                        <span
                          className="hm-mono text-[11px] ml-1"
          ...
          <div className="lg:col-span-9 flex flex-col gap-5">
            ...
            >
              <p className="hm-mono text-[12px]" style={{ color: 'var(--hm-text-muted)' }}>
                ...
                {activeCategory !== 'All' && (
                  <>
                    {' '}
                    · <span style={{ color: 'var(--hm-violet-2)' }}>{activeCategory}</span>
                  </>
                )}
              </p>
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  <button
                    ...
                    style={{
                      background: viewMode === 'grid' ? 'var(--hm-violet-soft)' : 'transparent',
                      color: viewMode === 'grid' ? 'var(--hm-violet-2)' : 'var(--hm-text-dim)',
                    }}
                  ...
                  <button
                    ...
                    className="flex h-7 w-7 items-center justify-center rounded transition-colors"
                    style={{
                      background: viewMode === 'list' ? 'var(--hm-violet-soft)' : 'transparent',
                      color: viewMode === 'list' ? 'var(--hm-violet-2)' : 'var(--hm-text-dim)',
                    }}
            ...
            <div className="mt-8 flex items-center justify-center gap-3">
              <button
                type="button"
                className="flex h-9 w-9 items-center justify-center rounded-full text-[12px] transition-colors"
                style={{
                  background: 'var(--hm-bg-card)',
                  border: '1px solid var(--hm-border)',
...
function FilterGroup({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <h3
        className="hm-mono text-[10px] mb-3 flex items-center justify-between"
...
function CheckboxRow({
  ...
  return (
    <button
      type="button"
      onClick={onChange}
      className="flex items-center justify-between gap-2 px-1 py-1 text-left rounded transition-colors"
    >
...
function CourseCard({ course, mode }: { course: Course; mode: 'grid' | 'list' }) {
  ...
  return (
    <a
      href="#"
      className={`hm-card hm-card-hover group flex ${isList ? 'flex-col sm:flex-row' : 'flex-col'} overflow-hidden cursor-pointer`}
    >
      {/* Cover */}
      <div
        className={`relative ${isList ? 'h-44 sm:h-auto sm:w-56 shrink-0' : 'h-36 w-full'} overflow-hidden`}
        style={{
          background:
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div
            ...
            style={{
              ...
              border: '1px solid var(--hm-border-accent)',
              boxShadow: '0 8px 28px -10px rgba(139, 92, 246, 0.40)',
            }}
          >
      ...
      <div className="p-5 flex flex-col flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-2">
          <span
            className="hm-mono text-[9.5px] font-semibold uppercase rounded px-1.5 py-0.5"
            style={{
              background: 'var(--hm-violet-soft)',
              color: 'var(--hm-violet-2)',
              border: '1px solid var(--hm-border-accent)',
        ...
        <h3
          className="text-[15.5px] font-semibold leading-snug mb-2 line-clamp-2"
        ...
        <p
          className={`text-[12.5px] mb-4 ${isList ? '' : 'line-clamp-2'} flex-1`}
          style={{ color: 'var(--hm-text-muted)', lineHeight: 1.55 }}
        >
          {course.description}
        </p>
        ...
        {/* Meta row */}
        <div
          className="flex flex-wrap items-center gap-x-3 gap-y-1 hm-mono text-[10.5px] mb-4"
          style={{ color: 'var(--hm-text-dim)' }}
        >
          <span className="inline-flex items-center gap-1">
            <Star
              className="w-3 h-3"
              style={{ color: 'var(--hm-amber)', fill: 'var(--hm-amber)' }}
            />
            <span style={{ color: 'var(--hm-text)' }}>{course.rating}</span>
        ...
        >
          <div className="flex items-center gap-2 min-w-0">
            <div
              className="w-6 h-6 rounded-full flex items-center justify-center text-[9.5px] font-semibold shrink-0"
              style={{
          ...
          <span
            ...
            style={{
              color: course.price === 'Free' ? 'var(--hm-violet-2)' : 'var(--hm-text)',
